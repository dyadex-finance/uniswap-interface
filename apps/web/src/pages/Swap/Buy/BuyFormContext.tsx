...
export const ethCurrencyInfo = buildPartialCurrencyInfo(nativeOnChain(UniverseChainId.Monad))
...
function useDerivedBuyFormInfo(state: BuyFormState): BuyInfo {
  ...
  const accountAddress = useActiveAddress(state.quoteCurrency?.currencyInfo?.currency.chainId ?? UniverseChainId.Monad)
